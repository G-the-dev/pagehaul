...
const st = await page.evaluate(`(()=>{
  ...
  const videos=[...document.querySelectorAll('[data-tile] video')];
  return { tilesMounted:tiles.length, videoElements:videos.length,
           decoding:videos.filter(v=>v.readyState>=2).length };
})()`);
console.log("video tab, at rest:", JSON.stringify(st), "-> videoElements should be ~visible count, not all mounted");
// throttled scroll frame pacing through the video grid
await cdp.send("Emulation.setCPUThrottlingRate",{rate:4});
await page.evaluate(`window.scrollTo(0,700)`); await new Promise(r=>setTimeout(r,1000));
const gaps = await page.evaluate(`new Promise(res=>{const out=[];let last=performance.now();let n=0;
  function f(now){out.push(now-last);last=now;window.scrollBy(0,80);
    if(++n<150) requestAnimationFrame(f); else res(out.slice(3));} requestAnimationFrame(f);})`);
const q=[...gaps].sort((a,b)=>a-b);
console.log(`scroll @4x — median ${q[Math.floor(q.length/2)].toFixed(1)}ms  p90 ${q[Math.floor(q.length*0.9)].toFixed(1)}ms  frames>50ms ${gaps.filter(g=>g>50).length}/${gaps.length}`);
await b.close();
